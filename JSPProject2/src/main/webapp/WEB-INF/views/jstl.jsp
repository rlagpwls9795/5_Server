...
<title>3. JSTL</title>
	<style>
		.first{background-color: yellow; color:red;}
...
<body>
	...
	<pre>
	JSP에서 자주 사용되거나 공통적으로 사용되는 Java 코드의 표기법릉 간단히 태그화하여 표준으로 제공하는 것. 
	
	(if, for, 배열/컬렉션 길이, 문자열 치환, 숫자 데이터 형식 변경, 데이터 파싱, scope 변수 선언)
	</pre>
	...
	- c:set 속성
	  ...
	  2) value : 대입될 값(setAttribute의 value 값)
	...
	<h4>\${num2} : ${num2}</h4> <!-- 20 (우선순위 page가 더 높음) --> 
	<h4>\${requestScope.num2} : ${requestScope.num2}</h4> <!-- 300 -->
	...
	- c:remove 속성
	  1) var : 삭제할 변수명(setAttribute의 key 값)
	...
	* 주의 사항 *
	...
	<pre>
	c:choose : when, otherwise를 감싸는 태그 (현재 태그 내부에 if ~ else if ~ else를 작성 예정)
	c:when : if / else if 를 나타내는 태그, 속성은 test밖에 없음 (조건 작성 속성)
	c:otherwise : else를 나태내는 태그, 아무런 속성도 존재하지 않음
	</pre>
