...
<body>
	...
	2. request : 요청 받은 페이지(Servlet 또는 JSP)와 요청을 위임 받은 페이지(Servlet 또는 JSP)에서 사용 가능
	<%-- <% request.getParameter("name"); %> --%>
	   (최소 2 페이지 이상)
	   
	3. session : 현재 사이트에 접속한 브라우저 당 1개씩 생성. 브라우저가 종료되거나 session이 만료될 때까지 유지
	   (세션에 로그인 정보를 기록해둠 -> 브라우저가 종료되거나 로그아웃이 되기 전까지 계속 로그인 상태 유지)
	...
	*****************
	-> setAttribute("key", value)로 내장 객체가 값을 세팅할 때 key 값이 중복되는 경우
	   ...
	   \${key}로 작성하는 경우 범위가 작은 내장 객체가 높은 우선순위를 갖게 된다.
